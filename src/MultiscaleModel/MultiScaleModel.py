            ...
            self.grouped.append(
                nn.Sequential(
                    nn.Conv3d(
                        ...
                        nn.GroupNorm(num_groups=4, num_channels=self.outChannels, eps=1e-01, affine=True),
                        nn.PReLU()
                    )
                )
...
'''
clahe和32fu很长时间不训练的机器或重开spyder似乎容易复现
'''


'''
最优结果是先150，然后在160,对于32fu
'''
epoch=150
folder='./dataset/'
...
device='cuda'
model=model(number=32,phase='train').to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-4,weight_decay=0)
# optimizer = torch.optim.SGD(model.parameters(), lr=1e-4,weight_decay=1e-4,momentum=0.9,nesterov=True)
# optimizer=adabound.AdaBound(model.parameters(),lr=1e-4,weight_decay=0,final_lr=0.01)
lrsch=lr_scheduler.ReduceLROnPlateau(optimizer,mode='max',factor=0.1,patience=30)
lossm=AvgMeter()
cost=SoftDiceLoss()
# cost=LogCoshDiceLoss()

#实际的训练方式是先训练150轮，之后看学习率变化情况，再决定是否150轮，然后继续。比如64的训练是150轮+150轮+150轮
...
for i in range(1,epoch+1):
    ...
    for x,y in trainloader:
        ...
        predict=model(x)
        # l1=F.binary_cross_entropy_with_logits(predict['out'], y, pos_weight= pPixelW)
        # l2=0.8*F.binary_cross_entropy_with_logits(predict['as1'], y, pos_weight= pPixelW)
        # l3=0.7*F.binary_cross_entropy_with_logits(predict['as2'], y, pos_weight= pPixelW)
        # l4=0.6*F.binary_cross_entropy_with_logits(predict['as3'], y, pos_weight= pPixelW)
        # l5=0.5*F.binary_cross_entropy_with_logits(predict['as4'], y, pos_weight= pPixelW)
        # l6=0.8*F.binary_cross_entropy_with_logits(predict['s1'], y, pos_weight= pPixelW)
        # l7=0.7*F.binary_cross_entropy_with_logits(predict['s2'], y, pos_weight= pPixelW)
        # l8=0.6*F.binary_cross_entropy_with_logits(predict['s3'], y, pos_weight= pPixelW)
        # l9=0.5*F.binary_cross_entropy_with_logits(predict['s4'], y, pos_weight= pPixelW)
    ...
    all_states = {"model": model.state_dict(), "adam": optimizer.state_dict(), "epoch":i}
    torch.save(all_states, "./multiscale.pth.gz")
